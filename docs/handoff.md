...
## 最新:2026-09-01 P2 批次一/二:覆盖率UI、输入限额、简化模式契约、服务端闸门

- **11.2.8 清洗覆盖率 UI**:导入完成的状态行明示「模型已复核 X 条 · 仅本地/存量规则 Y 条」,
  覆盖口径不再让用户误以为全量模型复核。
- **11.3.4 输入限额**:pipeline.parse 入口拦文件体积(>3000 万字符报错);
  normalize 拦条目数(>2000 截断告警)与字段长度(id 80/title 200/url 2048/folder 300)。
- **11.2.9 简化模式契约**:回退模板标题带【简化模式】+ level.simplified 标记 +
  premise 说明谜题深度低于正式生成;固定模板的 combine-pair 补 resultOn/product,
  deliver 改引 result:combine-result——回退关卡的交付也引用最终产物(契约兑现)。
- **11.1.4 余项**:LLM/抓取代理( /api/step、/api/glm、/fetch-meta )加并发闸门
  (BoundedSemaphore 8,满发 429)与频率限制(120 次/60s 窗口,超发 429);
  请求体上限此前已有(MAX_BODY 20MB)。一次性 token 暂不做:服务仅绑定 loopback
  且 CORS 仅本机来源,攻击面有限——如未来对外开放端口再引入。
- **11.3.3**:datasets 缓存键补 CLEAN_VERSION(清洗口径变化即失效)。
- **11.2.10 ending 文案与行为诚实化**:三选项改为「继续看看/归档/重玩本关」,
  只承诺真实行为;「重玩本关」接入产品层 resetCurrentLevel 真实重置(保留存档记录)。
- 验证:频率闸门实测(130 连发出现 11 次 429)、正常请求 400/200 分流正确、
  design race 15/15(含回退路径「6 次调用后退回固定模板挂载」)、smoke 无控制台错误。
...
1. **prompt 同步**:ea65645 把【手法菜单】+【答案与文案铁律】只写进了回退路径
   (callStepLevel:578),主路 designWindow(3085+)没教——但 P67 答案分布/knock 配额
   等机器检查全链路生效,模型盲踩不知道的门。已把两段同步进主路 systemPrompt
   (第 4/5 条),回退 prompt 的「全关恰好一个推理锁」改为「1-2 个(检索多问可用两把
   password 挂同一 lock)」消掉与手法 #2 的矛盾。
2. **接地口径漏洞(goal-after-sync.log 周期1 实证)**:sourceFacts 接地检查的对照文本
   只有 desc/标题/路径,不含域名与日期——而域名/日期是物件弹窗常驻展示的真实字段
   (P67 措辞铁律要求谜面与展示一致),模型引用「域名:nandgame.com」被误判编造,
   实测烧光周期 1 的全部赛马轮次(224s 超时)。已把 domain+dateAdded 补进 ground
   文本(designWindow 与 goal 测试断言两处口径同步)。
3. **实测(goal-after-domain-fix.log)**:两周期 **36/36 全绿**,周期 1 全新生成
   挂载成功,desc 4-6/9、digest 6-9、事实接地 0 违规。门禁未放松一条——修的是
   「模型被冤枉」的那一条。
