              ...
              beats: [
                {
                  id: 'step id',
                  title: '可观察的步骤',
                  action: 'inspect|combine|revisit|sequence|deliver|password|angle|morse',
                  uses: ['素材 id 或 result:前置组合步id'],
                  requires: ['前置 step id'],
                  reveals: ['本步完成后进入待发现状态的隐藏素材 id'],
  ...
  function compileLevel(draft, design) {
    ...
    if (design && Array.isArray(design.scenes) && design.scenes.length) {
      ...
      design.scenes.slice(0, 4).forEach(function (sc, si) {
        ...
        /* 推理锁参数清洗与全局配额:参数不合法降级为 inspect;password≤2/angle≤2/morse≤1 防滥用 */
        const seen = { password: 0, angle: 0, morse: 0 };
            ...
            scenesIn.forEach(function (sc, si) {
              ...
              items.forEach(function (it) {
                ...
                /* 接地检查(P46/P62):sourceFacts 的值必须能在该素材的可核查文本
                   (desc/标题/路径)里原样找到——把"内容参与解谜"变成机器可验证,
